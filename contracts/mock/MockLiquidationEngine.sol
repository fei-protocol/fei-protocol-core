...
contract MockLiquidationEngine {
    ...
    function connectSAFESaviour(address saviour) external isAuthorized {
        (bool ok, uint256 collateralAdded, uint256 liquidatorReward) = SAFESaviourLike(saviour).saveSAFE(
            address(this),
            "",
            address(0)
        );
        ...
        require(
            both(collateralAdded == type(uint256).max, liquidatorReward == type(uint256).max),
    ...
    function liquidateSAFE(bytes32 collateralType, address safe) external returns (uint256 auctionId) {
        ...
        (, uint256 accumulatedRate, , , uint256 debtFloor, uint256 liquidationPrice) = safeEngine.collateralTypes(
            collateralType
        );
        ...
        if (
            ...
            try
                SAFESaviourLike(chosenSAFESaviour[collateralType][safe]).saveSAFE(msg.sender, collateralType, safe)
        ...
        if (
            ...
            uint256 limitAdjustedDebt = minimum(
                ...
                multiply(
                    ...
                    accumulatedRate /
                    collateralData.liquidationPenalty
            );
...

            {
                ...
                uint256 amountToRaise_ = multiply(
