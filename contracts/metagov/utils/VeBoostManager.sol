...
/// @author eswak
abstract contract VeBoostManager is CoreRef {
    // events
    ...
    /// @notice constructor
    /// @param _votingEscrowDelegation the contract managing veboost delegation
    constructor(address _votingEscrowDelegation) {
    ...
    function setVotingEscrowDelegation(address newVotingEscrowDelegation)
        public
        whenNotPaused
    ...
    function create_boost(
        ...
        uint256 _expire_time,
        uint256 _id
    ) external whenNotPaused onlyTribeRole(TribeRoles.METAGOVERNANCE_TOKEN_STAKING) {
    ...
    /// @notice Cancel an outstanding boost
    function cancel_boost(uint256 _token_id)
        external
        whenNotPaused
        onlyTribeRole(TribeRoles.METAGOVERNANCE_TOKEN_STAKING)
    {
    ...
    /// @notice Destroy a token
    function burn(uint256 _token_id) external whenNotPaused onlyTribeRole(TribeRoles.METAGOVERNANCE_TOKEN_STAKING) {
