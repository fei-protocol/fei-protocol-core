...
contract NonCustodialPriceBoundPSMTest is DSTest {
    ...
    uint256 public constant price_floor = 500;
    uint256 public constant price_ceiling = 20000;
    ...
    function setUp() public {
        ...
        NonCustodialPriceBoundPSM.OracleParams
            memory oracleParams = NonCustodialPSM.OracleParams({
        ...
        NonCustodialPriceBoundPSM.RateLimitedParams
        ...
        NonCustodialPriceBoundPSM.PSMParams memory PSMParams = NonCustodialPSM
            .PSMParams({
    ...
    */
    ...
    /// @notice mint fails when price is too high
    function testMintFailsWhenPriceTooHigh() public {
        vm.startPrank(addresses.governorAddress);
        psm.setOracleCeilingBasisPoints(5000);
        vm.stopPrank();

        vm.expectRevert(bytes("PegStabilityModule: price out of bounds"));
        psm.mint(address(this), 100, 101);
    }

    /// @notice mint fails when price is too low
    function testMintFailsWhenPriceTooLow() public {
        vm.startPrank(addresses.governorAddress);
        psm.setOracleFloorBasisPoints(15000);
        vm.stopPrank();
    ...
    /// @notice redeem fails when price is too high
    function testRedeemFailsWhenPriceTooHigh() public {
        vm.startPrank(addresses.governorAddress);
        psm.setOracleCeilingBasisPoints(5000);
        vm.stopPrank();
    ...
    /// @notice redeem fails when price is too low
    function testRedeemFailsWhenPriceTooLow() public {
        vm.startPrank(addresses.governorAddress);
        psm.setOracleFloorBasisPoints(15000);
        vm.stopPrank();
