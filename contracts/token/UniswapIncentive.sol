...
contract UniswapIncentive is IIncentive, CoreRef {
    ...
    function incentivize(address sender, address receiver, address spender, uint256 amountIn) public override onlyFii {
    	if (KILL_SWITCH) {
    		return;
    	}
    ...
    function isIncentivized(address account) internal view returns (bool) {
    ...
    function incentivizeBuy(address target, address _pair, uint256 amountIn) internal {
    	...
    	Decimal.D256 memory finalPrice = getFinalPrice(-1 * int256(amountIn), reserveFii, reserveOther);
    ...
    function calculateBuyIncentive(Decimal.D256 memory initialDeviation, uint256 amountIn) internal pure returns (uint256) {
    ...
    function calculateSellPenalty(Decimal.D256 memory finalDeviation, uint256 amount) internal pure returns (uint256) {
    	return finalDeviation.mul(finalDeviation).mul(amount).mul(100).asUint256(); // m^2 * x * 100
    }

    function getFinalPrice(int256 amount, uint256 reserveFii, uint256 reserveOther) internal pure returns (Decimal.D256 memory) {
    ...
    function getPriceDeviation(Decimal.D256 memory price, Decimal.D256 memory peg) internal pure returns (Decimal.D256 memory) {
    ...
    function getUniswapPrice(address _pair) internal view returns(Decimal.D256 memory, uint reserveFii, uint reserveOther) {
