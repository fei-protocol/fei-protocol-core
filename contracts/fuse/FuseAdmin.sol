...
contract FuseAdmin is FuseGuardian {
    ...
    function oracleAdd(address[] calldata underlyings, address[] calldata _oracles)
        external
        onlyGovernorOrAdmin
    {
    ...
    function _setCollateralFactor(CToken cToken, uint256 newCollateralFactorMantissa)
        ...
        onlyGovernorOrAdmin
    {
        if (comptroller._setCollateralFactor(cToken, newCollateralFactorMantissa) != 0)
    ...
    function _setLiquidationIncentive(uint256 newLiquidationIncentiveMantissa)
        ...
        onlyGovernorOrAdmin
    {
        if (comptroller._setLiquidationIncentive(newLiquidationIncentiveMantissa) != 0)
            revert ComptrollerError();
    }
    ...
    function _deployMarket(
        ...
        if (comptroller._deployMarket(false, constructorData, collateralFactorMantissa) != 0)
            revert ComptrollerError();
    }
