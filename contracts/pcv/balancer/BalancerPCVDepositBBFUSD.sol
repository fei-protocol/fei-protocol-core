...
contract BalancerPCVDepositBBFUSD is PCVDeposit {
    ...
    constructor(address _core, uint256 _maximumSlippageBasisPoints)
        CoreRef(_core)
    {
    ...
    function balance() public view override returns (uint256) {
        uint256 bptBalance = IPhantomStablePool(BB_F_USD_ADDRESS).balanceOf(
            address(this)
        );
    ...
    function depositUnwrapped(uint256 maxTokens)
        external
        whenNotPaused
        onlyPCVController
    {
        ...
        require(
            stablecoinsDeposited != 0,
            "BalancerPCVDepositBBFUSD: Deposit 0"
        );

        // For each stablecoin, perform a batch swap to get bb-f-USD
        // e.g. FEI ---> bb-f-FEI ---> bb-f-USD
        uint256 bbfUsdBalanceBefore = IPhantomStablePool(BB_F_USD_ADDRESS)
        ...
        // check for slippage
        uint256 bbfUsdBalanceAfter = IPhantomStablePool(BB_F_USD_ADDRESS)
            .balanceOf(address(this));
        uint256 bbfUsdReceived = bbfUsdBalanceAfter - bbfUsdBalanceBefore;
        uint256 usdReceived = (bbfUsdReceived *
            IPhantomStablePool(BB_F_USD_ADDRESS).getRate()) / 1e18;
        uint256 minUsdReceived = (stablecoinsDeposited *
            (Constants.BASIS_POINTS_GRANULARITY - maximumSlippageBasisPoints)) /
            Constants.BASIS_POINTS_GRANULARITY;
        require(
            usdReceived >= minUsdReceived,
            "BalancerPCVDepositBBFUSD: slippage too high"
        );
    ...
    function deposit() external override whenNotPaused {
        ...
        if (dai > 0) {
            ...
            uint256 erc4626Dai = IERC4626(ERC4626_DAI).deposit(
                dai,
                address(this)
            );
        ...
        if (lusd > 0) {
            ...
            uint256 erc4626Lusd = IERC4626(ERC4626_LUSD).deposit(
                lusd,
                address(this)
            );
        ...
        if (fei > 0) {
            ...
            uint256 erc4626Fei = IERC4626(ERC4626_FEI).deposit(
                fei,
                address(this)
            );
    ...
    function withdrawBatchSwap(
        ...
        if (linearPoolId == BB_F_DAI_POOLID) {
            require(
                linearPoolAddress == BB_F_DAI_ADDRESS,
                "BalancerPCVDepositBBFUSD: wrong pool address"
            );
            if (unwrap)
                require(
                    tokenAddress == ERC4626_DAI,
                    "BalancerPCVDepositBBFUSD: wrong wrapped token"
                );
            else
                require(
                    tokenAddress == ERC20_DAI,
                    "BalancerPCVDepositBBFUSD: wrong token"
                );
        } else if (linearPoolId == BB_F_LUSD_POOLID) {
            require(
                linearPoolAddress == BB_F_LUSD_ADDRESS,
                "BalancerPCVDepositBBFUSD: wrong pool address"
            );
            if (unwrap)
                require(
                    tokenAddress == ERC4626_LUSD,
                    "BalancerPCVDepositBBFUSD: wrong wrapped token"
                );
            else
                require(
                    tokenAddress == ERC20_LUSD,
                    "BalancerPCVDepositBBFUSD: wrong token"
                );
        } else if (linearPoolId == BB_F_FEI_POOLID) {
            require(
                linearPoolAddress == BB_F_FEI_ADDRESS,
                "BalancerPCVDepositBBFUSD: wrong pool address"
            );
            if (unwrap)
                require(
                    tokenAddress == ERC4626_FEI,
                    "BalancerPCVDepositBBFUSD: wrong wrapped token"
                );
            else
                require(
                    tokenAddress == ERC20_FEI,
                    "BalancerPCVDepositBBFUSD: wrong token"
                );
        } else {
            revert("BalancerPCVDepositBBFUSD: wrong linear pool");
        }
        ...
        uint256 maxBbfUsdSpent = (tokenAmount *
            1e18 *
            (Constants.BASIS_POINTS_GRANULARITY + maximumSlippageBasisPoints)) /
            (IPhantomStablePool(BB_F_USD_ADDRESS).getRate() *
                Constants.BASIS_POINTS_GRANULARITY);
        ...
        if (unwrap) {
            IERC4626(tokenAddress).redeem(
                IERC20(tokenAddress).balanceOf(address(this)),
                address(this),
                address(this)
            );
            IERC20(IERC4626(tokenAddress).asset()).transfer(
                tokenDestination,
                tokenAmount
            );
        }
