...
import {IFeiRouter} from "./IFeiRouter.sol";
...
contract FeiRouter is IFeiRouter {
    ...
    function buyFei(
        ...
        require(amountOut >= amountOutMin, "FeiRouter: Insufficient output amount");
        ...
        (uint256 amount0Out, uint256 amount1Out) = isWETHPairToken0 ? (uint256(0), amountOut) : (amountOut, uint256(0));
        ...
        uint256 reward = feiBalanceAfter - feiBalanceBefore - amountOut;
    ...
    function sellFei(
        ...
        uint256 effectiveAmountIn = IERC20(fei).balanceOf(address(PAIR)) - reservesOther;
        ...
        amountOut = UniswapV2Library.getAmountOut(effectiveAmountIn, reservesOther, reservesETH);
        require(amountOut >= amountOutMin, "FeiRouter: Insufficient output amount");

        (uint256 amount0Out, uint256 amount1Out) = isWETHPairToken0 ? (amountOut, uint256(0)) : (uint256(0), amountOut);
        ...
        )
    {
        ...
        return isWETHPairToken0 ? (reserves0, reserves1, isWETHPairToken0) : (reserves1, reserves0, isWETHPairToken0);
    }
}
