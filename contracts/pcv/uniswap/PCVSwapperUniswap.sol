...
contract PCVSwapperUniswap is IPCVSwapper, UniRef, Timed {
    ...
    event WithdrawERC20(address indexed _caller, address indexed _to, address indexed _token, uint256 _amount);
    ...
    function swap() external override afterTime whenNotPaused {
        if (tokenBuyLimit != 0) {
            require(
                ERC20(tokenReceived).balanceOf(tokenReceivingAddress) < tokenBuyLimit,
                "PCVSwapperUniswap: tokenBuyLimit reached."
            );
        }
        ...
        (uint256 amount0Out, uint256 amount1Out) = pair.token0() == address(tokenSpent)
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));
        pair.swap(amount0Out, amount1Out, tokenReceivingAddress, new bytes(0));

        // Emit event
        emit Swap(msg.sender, tokenSpent, tokenReceived, amountIn, amountOut);
    ...
    function _getExpectedAmountOut(uint256 amountIn) internal view returns (uint256) {
        ...
        (uint256 tokenSpentReserves, uint256 tokenReceivedReserves) = pair.token0() == tokenSpent
            ? (_token0, _token1)
            : (_token1, _token0);

        // Prepare swap
        uint256 amountOut = UniswapV2Library.getAmountOut(amountIn, tokenSpentReserves, tokenReceivedReserves);
