...
contract UniswapV3OracleWrapper is IUniswapV3OracleWrapper, IOracle, CoreRef {
    ...
    IUniswapWrapper private immutable uniswapWrapper;
    ...
    constructor(
        ...
        require(
            _oracleConfig.uniswapPool != address(0x0),
            "_pool cannot be null address"
        );
        require(
            _uniswapWrapper != address(0x0),
            "_uniswapWrapper cannot be null address"
        );

        validatePoolLiquidity(
            _oracleConfig.uniswapPool,
            _oracleConfig.minPoolLiquidity
        );
        validateTokensInPool(
            _oracleConfig.uniswapPool,
            _inputToken,
            _outputToken
        );
    ...
    function getTwapPeriod() external view override returns (uint32) {
    ...
    function validatePoolLiquidity(address _pool, uint128 _minPoolLiquidity)
        internal
        view
    {
        require(
    ...
    function validateTokensInPool(
        ...
        (address token0, address token1) = sortTokensAccordingToUniswap(
            _inputToken,
            _outputToken
        );
    ...
    function addSupportForPool(
        address _pool,
        uint32 _twapPeriod,
        uint32 _meanBlockTime
    ) internal {
        uint16 requiredCardinality = uint16(_twapPeriod / _meanBlockTime) +
    ...
    function sortTokensAccordingToUniswap(address tokenA, address tokenB)
        internal
        pure
    ...
    function setTwapPeriod(uint32 _twapPeriod)
        external
        override
        hasAnyOfThreeRoles(
            TribeRoles.GOVERNOR,
            TribeRoles.GUARDIAN,
            TribeRoles.ORACLE_ADMIN
        )
    {
