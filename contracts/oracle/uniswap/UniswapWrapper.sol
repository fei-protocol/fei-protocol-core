...
contract UniswapWrapper {
    ...
    function calculatePrice(
        ...
        (
            uint256 decimalNormaliser,
            bool invertDecNormaliser
        ) = calculateDecimalNormaliser(inputTokenDecimals, outputTokenDecimals);
    ...
    function getUnNormalisedPrice(uint32 _twapPeriod, address _pool)
        internal
        view
        returns (uint256)
    {
    ...
    function sortTokensAccordingToUniswap(
        address inputToken,
        address outputToken
    ...
    function calculateDecimalNormaliser(
        ...
        if (_token0Decimals >= _token1Decimals) {
            ...
            return (
                10**(_token0Decimals - _token1Decimals),
                invertDecNormaliser
            );
        } else {
            invertDecNormaliser = true;
            return (
                10**(_token1Decimals - _token0Decimals),
                invertDecNormaliser
            );
        }
    }
