...
describe('FeiTimedMinter', function () {
  ...
  beforeEach(async function () {
    ...
    this.feiMinter = await (await ethers.getContractFactory('FeiTimedMinter')).deploy(
      this.core.address,
      userAddress, 
      this.incentive, 
      this.frequency, 
  ...
  describe('Set Target', function() {
    ...
    it('non-governor reverts', async function() {
      await expectRevert(this.feiMinter.connect(impersonatedSigners[userAddress]).setTarget(secondUserAddress), 'CoreRef: Caller is not a governor');
    });
  
    it('zero address reverts', async function() {
      await expectRevert(this.feiMinter.connect(impersonatedSigners[governorAddress]).setTarget(ZERO_ADDRESS), 'FeiTimedMinter: zero address');
    });
  });
  ...
  describe('Set Mint Amount', function() {
    ...
    it('non-governor reverts', async function() {
      await expectRevert(this.feiMinter.connect(impersonatedSigners[userAddress]).setMintAmount('500'), 'CoreRef: Caller is not a governor');
    });
  });
  ...
  describe('Set Frequency', function() {
    ...
    it('non-governor reverts', async function() {
      await expectRevert(this.feiMinter.connect(impersonatedSigners[userAddress]).setFrequency('5000'), 'CoreRef: Caller is not a governor');
    });
    ...
    it('above max reverts', async function() {
      await expectRevert(this.feiMinter.connect(impersonatedSigners[governorAddress]).setFrequency('5000000'), 'FeiTimedMinter: frequency high');
    });
  });
});
