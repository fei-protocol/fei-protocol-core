...
describe('EthBondingCurve', function () {
  ...
  beforeEach(async function () {
    ...
    const mockOracleFactory = await ethers.getContractFactory(MockOracle.abi, MockOracle.bytecode);
    this.oracle = await mockOracleFactory.deploy(500); // 500 USD per ETH exchange rate 

    const mockEthPCVDepositFactory = await ethers.getContractFactory(MockEthPCVDeposit.abi, MockEthPCVDeposit.bytecode);
    this.pcvDeposit1 = await mockEthPCVDepositFactory.deploy(beneficiaryAddress1);
    this.pcvDeposit2 = await mockEthPCVDepositFactory.deploy(beneficiaryAddress2);
  ...
  describe('Allocate', function() {
    ...
    describe('With Purchase', function() {
      ...
      it('splits funds', async function() {
        expect(await balance.current(beneficiaryAddress1)).to.be.bignumber.equal(this.beneficiaryBalance1.add(toBN('9000000')));
        expect(await balance.current(beneficiaryAddress2)).to.be.bignumber.equal(this.beneficiaryBalance2.add(toBN('1000000')));
      });
      ...
      describe('Second Allocate', async function() {
        ...
        describe('Updated Allocation', function() {
          ...
          it('splits funds', async function() {
            expect(await balance.current(beneficiaryAddress1)).to.be.bignumber.equal(this.beneficiaryBalance1.add(toBN('5000000')));
            expect(await balance.current(beneficiaryAddress2)).to.be.bignumber.equal(this.beneficiaryBalance2.add(toBN('5000000')));
          });
