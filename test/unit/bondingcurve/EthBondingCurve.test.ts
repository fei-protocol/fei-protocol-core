...
describe('EthBondingCurve', function () {
  ...
  beforeEach(async function () {
    ...
    this.bondingCurve = await this.bondingCurveFactory.deploy(
      this.core.address,
      this.oracle.address,
      this.oracle.address,
      {
  ...
  describe('Purchase', function () {
    ...
    describe('Incorrect ETH sent', function () {
      it('Too little ETH', async function () {
        await expectRevert(
          this.bondingCurve.purchase(userAddress, this.purchaseAmount, { value: '100' }),
          'Bonding Curve: Sent value does not equal input'
        );
      });
      it('Too much ETH', async function () {
        await expectRevert(
          this.bondingCurve.purchase(userAddress, this.purchaseAmount, { value: '1000000000000000000' }),
          'Bonding Curve: Sent value does not equal input'
        );
      });
    });
        ...
        it('reverts', async function () {
          ...
          await expectRevert(
            this.bondingCurve
              ...
              .purchase(userAddress, this.purchaseAmount, { value: this.purchaseAmount }),
  ...
  describe('Allocate', function () {
    ...
    describe('With Purchase', function () {
      ...
      it('splits funds', async function () {
        ...
        expect(await ethers.provider.getBalance(beneficiaryAddress2)).to.be.equal(
