...
describe('ConvexPCVDeposit', function () {
  ...
  describe('stakeLpTokens()', function () {
    it('should stake LP tokens held on the contract', async function () {
      expect(await curvePool.balanceOf(convexReward.address)).to.be.equal('0');
      await curvePool.transfer(deposit.address, '100');
      await deposit.stakeLpTokens();
      expect(await curvePool.balanceOf(convexReward.address)).to.be.equal('100');
    });
  });
