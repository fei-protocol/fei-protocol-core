...
describe('StableSwapOperatorV1', function () {
  ...
  beforeEach(async function () {
    ({ userAddress, governorAddress, minterAddress, burnerAddress, pcvControllerAddress } = await getAddresses());
    ...
    this.mock3pool = await (
      await ethers.getContractFactory('MockCurve3pool')
    ).deploy(this.dai.address, this.usdc.address, this.usdt.address);

    this.mockMetapool = await (
      await ethers.getContractFactory('MockCurveMetapool')
    ).deploy(this.mock3pool.address, this.fei.address);

    this.deposit = await (
      await ethers.getContractFactory('StableSwapOperatorV1')
    ).deploy(
      this.core.address,
    ...
    await this.mock3pool
      .connect(impersonatedSigners[userAddress])
      .add_liquidity([`300000${e18}`, `300000${e18}`, `300000${e18}`], 0, {});
    ...
    await this.mock3pool
      .connect(impersonatedSigners[userAddress])
      .approve(this.mockMetapool.address, `900000${e18}`, {});
    await this.mockMetapool
      .connect(impersonatedSigners[userAddress])
      .add_liquidity([`900000${e18}`, `900000${e18}`], 0, {});
  });
  ...
  describe('deposit()', function () {
    it('should emit Deposit event on success', async function () {
      ...
      await expect(await this.deposit.connect(impersonatedSigners[pcvControllerAddress]).deposit()).to.emit(
        this.deposit,
        'Deposit'
      ); //.withArgs('49992501124831275308703694')
    ...
    it('should revert if not enough FEI for tokens held', async function () {
      ...
      await expectRevert(
        this.deposit.connect(impersonatedSigners[pcvControllerAddress]).deposit({}),
    ...
    it('should revert if slippage is too high in metapool', async function () {
      ...
      await expectRevert(
        this.deposit.connect(impersonatedSigners[pcvControllerAddress]).deposit({}),
        'StableSwapOperatorV1: metapool deposit slippage too high'
      );
    });
    it('should revert if slippage is too high in 3pool', async function () {
      ...
      await expectRevert(
        this.deposit.connect(impersonatedSigners[pcvControllerAddress]).deposit({}),
        'StableSwapOperatorV1: 3pool deposit slippage too high'
      );
    });
  });
  ...
  describe('withdraw()', function () {
    beforeEach(async function () {
      ...
      await this.mockMetapool
        .connect(impersonatedSigners[userAddress])
        .transfer(this.deposit.address, `6000${e18}`, {});
    });
    it('should emit Withdrawal event on success', async function () {
      ...
      await expect(
        ...
        .to.emit(this.deposit, 'Withdrawal')
        .withArgs(pcvControllerAddress, userAddress, `1000${e18}`);

      expect(await this.dai.balanceOf(userAddress)).to.be.equal(`1000${e18}`);
    });
    it('should revert if not PCVController', async function () {
      await expectRevert(
        this.deposit.connect(impersonatedSigners[userAddress]).withdraw(userAddress, `1000${e18}`, {}),
        'CoreRef: Caller is not a PCV controller'
      );
    });
    it('should revert if amount=0', async function () {
      await expectRevert(
        this.deposit.connect(impersonatedSigners[pcvControllerAddress]).withdraw(userAddress, `0${e18}`, {}),
        'StableSwapOperatorV1: Cannot withdraw 0'
      );
    });
    it('should not transfer tokens if target=self', async function () {
      ...
      await this.deposit
        ...
        .withdraw(this.deposit.address, `2000${e18}`, {});
    ...
    it('should revert if trying to withdraw amount > balance', async function () {
      await expectRevert(
        this.deposit.connect(impersonatedSigners[pcvControllerAddress]).withdraw(userAddress, `123456789${e18}`, {}),
    ...
    it('should revert if slippage is too high in 3pool', async function () {
      await this.mock3pool.set_slippage('1000'); // 10% slippage
      await expectRevert(
        this.deposit.connect(impersonatedSigners[pcvControllerAddress]).withdraw(userAddress, `1000${e18}`, {}),
        'StableSwapOperatorV1: 3pool withdraw slippage too high'
      );
    });
