...
describe('ERC20TokemakPCVDeposit', function () {
  ...
  beforeEach(async function () {
    ...
    this.pool = await (await ethers.getContractFactory('MockTokemakERC20Pool')).deploy(this.token.address);
    this.rewards = await (await ethers.getContractFactory('MockTokemakRewards')).deploy(this.toke.address);
    this.deposit = await (
      await ethers.getContractFactory('ERC20TokemakPCVDeposit')
    ).deploy(this.core.address, this.pool.address, this.rewards.address);

    this.depositAmount = toBN('10000000000000000000'); // 10 token
  });
  ...
  describe('Deposit', function () {
    ...
    it('succeeds if not paused', async function () {
      ...
      await this.token.mint(this.deposit.address, this.depositAmount);
    ...
    it('reverts if not PCVController', async function () {
      await expectRevert(
        this.deposit
          ...
          .withdrawERC20(this.token.address, userAddress, this.depositAmount, {}),
        'CoreRef: Caller is not a PCV controller'
      );
    });
