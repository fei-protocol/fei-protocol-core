import { expectRevert, balance, getAddresses, getCore } from '@test/helpers';
...
describe('EthCompoundPCVDeposit', function () {
  ...
  beforeEach(async function () {
    ...
    this.compoundPCVDeposit = await (
      await ethers.getContractFactory('EthCompoundPCVDeposit')
  ...
  describe('WithdrawERC20', function () {
    ...
    describe('From PCVController', function () {
      beforeEach(async function () {
        await (
          await ethers.getSigner(userAddress)
        ).sendTransaction({ from: userAddress, to: this.compoundPCVDeposit.address, value: this.depositAmount });
        await this.compoundPCVDeposit.deposit();
      });
