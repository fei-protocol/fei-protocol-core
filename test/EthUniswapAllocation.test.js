...
describe('EthUniswapAllocation', function () {
  ...
  beforeEach(async function () {
    ...
    await this.pair.set(100000, 50000000, LIQUIDITY_INCREMENT, {from: userAddress, value: 100000}); // 500:1 FII/ETH with 10k liquidity
  });
  ...
  describe('Deposit', function() {
    ...
    describe('Pre deposit values', function() {
      ...
      it('pair reserves', async function() {
        ...
        expect(await this.fii.balanceOf(this.pair.address)).to.be.bignumber.equal(new BN(50000000));
    ...
    describe('Post deposit values', function() {
      ...
      describe('No existing liquidity', function() {
        ...
        it('pair reserves', async function() {
          ...
          expect(await this.fii.balanceOf(this.pair.address)).to.be.bignumber.equal(new BN(100000000));
          let result = await this.allocation.getReserves();
        ...
        it('no fii held', async function() {
          expect(await this.fii.balanceOf(this.allocation.address)).to.be.bignumber.equal(new BN(0));
        });
      });      
      describe('With existing liquidity', function() {
        ...
        it('pair reserves', async function() {
          ...
          expect(await this.fii.balanceOf(this.pair.address)).to.be.bignumber.equal(new BN(150000000));
          let result = await this.allocation.getReserves();
      ...
      describe('After price move', function() {
        beforeEach(async function() {
          ...
          await this.fii.mint(this.pair.address, 100000000, {from: minterAddress});  
        ...
        it('pair reserves', async function() {
          expect(await balance.current(this.pair.address)).to.be.bignumber.equal(new BN(300000));
          expect(await this.fii.balanceOf(this.pair.address)).to.be.bignumber.equal(new BN(300000000));
          let result = await this.allocation.getReserves();
    ...
    describe('With Balance', function() {
      ...
      describe('Partial', function() {
        ...
        it('pair balances update', async function() {
          ...
          expect(await this.fii.balanceOf(this.pair.address)).to.be.bignumber.equal(new BN(75000000));
          let result = await this.allocation.getReserves();
