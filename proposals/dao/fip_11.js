...
async function validate(addresses, oldContracts, contracts) {
    ...
    for (let i = 0; i < pools.length; i++) {
        ...
        let kink = await contract.kink();
        let jumpMultiplierPerBlock = await contract.jumpMultiplierPerBlock();
        let multiplierPerBlock = await contract.multiplierPerBlock()
        let blocksPerYear = await contract.blocksPerYear();
...
module.exports = { setup, run, teardown, validate };
