...
const validate: ValidateUpgradeFunc = async (addresses, oldContracts, contracts, logging) => {
  ...
  expect(
    '0x' + (await ethers.provider.getStorageAt(addresses.balancerGaugeStaker, PROXY_ADMIN_STORAGE_SLOT)).slice(26)
  ).to.be.equal(vebalOtcBuyer.toLowerCase());

  // Check veBAL OTC Helper contract behavior

  // Can proxy.changeAdmin() to transfer balancerGaugeStaker proxy admin to someone else
