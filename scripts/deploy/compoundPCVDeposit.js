...
async function deploy(deployAddress, addresses, logging = false) {
  const { coreAddress, feiAddress, rariPool8FeiAddress, compoundEthAddress } = addresses;

  if (!coreAddress || !feiAddress || !rariPool8FeiAddress || !compoundEthAddress) {
  ...
  const compoundEthPCVDeposit = await EthCompoundPCVDeposit.new(coreAddress, compoundEthAddress, {
    from: deployAddress
  });
